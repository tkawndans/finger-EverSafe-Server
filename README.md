...
| `LOG_RETENTION_DAYS` | `10` | **보관 주기(일)**. `log/` 아래 **`serverlog-*.log`**, 구형 **`logYYYYMMDD.log`**, **`evaluate-warm-payload-*.log`** 중 **mtime** 기준으로 이 값보다 오래된 파일만 삭제합니다. **`0`** 이면 자동 삭제 안 함. 활성 **`serverlog.log`** 는 삭제하지 않습니다. |
