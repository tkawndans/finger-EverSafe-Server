...
| 단계 | 위치 | 동작 |
|------|------|------|
| **인코딩(클라이언트)** | 연동 앱·브라우저 | `JSON.stringify` 한 문자열을 **UTF-8 옥텟**으로 변환한 뒤(`stringToUtf8Bytes`), **65자 알파벳**(앞 64자 데이터 + 65번째 패딩)으로 Base64형 4문자 블록 인코딩합니다. 알파벳은 서버 **`BROWSER_ADMIN_TOKEN`** 과 동일(공백·개행 제거 후 길이 65자). |
| **디코딩(서버)** | `lib/payloadCustomBase64.js` | `decodeCustomBase64`로 옥텟을 복원한 뒤, `{`(0x7B)부터 **UTF-8 디코드**하여 문자열을 만듭니다. `JSON.parse`에 성공하면 그대로 사용합니다. **구형**(`encodeURIComponent`→ASCII 바이트)으로 만든 암호문은 `JSON.parse` 실패 시 예전 경로(라틴-1 조립·`decodeURIComponent`)로 **폴백**합니다. 최종 문자열을 `JSON.parse` 하여 객체로 씁니다. |
...
| HTTP | 조건 |
|------|------|
| 400 | `url` / `vmLoadBaseUrl` 포함, `payload`가 문자열이 아님, 디코드 실패, 디코드 결과의 `JSON.parse` 실패, `BROWSER_ADMIN_TOKEN` 길이 불일치 등. 응답 메시지는 운영에서 추측을 줄이도록 일반화되어 있을 수 있음. |
| 503 | 웜 비활성, 또는 `warmReady` 아님, 구성 진행 중 등. |

**`fetch` 예시** — 클라이언트에서 **먼저** `encodeCustomBase64(jsonString, alphabet)` 로 문자열을 만든 뒤 보냅니다. (인코더는 `lib/payloadCustomBase64.js` 와 동일 알고리즘이어야 합니다.)
...
| **evaluate/warm 페이로드** | JSON 본문의 `payload`만 **커스텀 Base64 문자열** 허용. `BROWSER_ADMIN_TOKEN`(트림 65자)이 알파벳. `lib/payloadCustomBase64.js` — [상세](#evaluate-warm-payload-encoding) |
