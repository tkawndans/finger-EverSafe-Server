...
| `LOG_EVALUATE_WARM_PAYLOAD` | (기본 기록) | **미설정이면 기본으로 기록**합니다. **`0`**(트림 후)만 두면 **`GET /health`** 스냅샷과 **`/evaluate/warm`** 관련(디코드 요약·XHR·HTTP)을 **터미널 + `process.cwd()/log/serverlog.log`** 에 남기지 않습니다. 그 외 값도 기록 켜짐. **`t`는 한국시간 `YYYY-MM-DD HH:mm:ss.SSS`**. 한국 자정마다 `serverlog.log` → `serverlog-YYYYMMDD.log` 로 롤링. 민감 정보 주의, 끌 때 **`=0`**. |
