...
│   └── warm.js            # POST …/evaluate/warm(+응답 `health`) , POST …/warm/retry
├── lib/
│   ├── healthPayload.js   # GET /health 본문·POST …/evaluate/warm 의 `health` 공통 생성
...
| `LOG_EVALUATE_WARM_PAYLOAD` | (기본 기록) | **미설정이면 기본으로 기록**합니다. **`0`**(트림 후)만 두면 **`GET /health`** 스냅샷과 **`/evaluate/warm`**·**`/warm/retry`** 관련 NDJSON(디코드 요약·XHR·HTTP **`http_request` / `http_response` / `http_error`**) 및 **`evaluate/warm` 자동 웜 복구** 시 `phase: "warm_auto_recover"` 줄을 **터미널 + `process.cwd()/log/serverlog.log`** 에 남기지 않습니다. 그 외 값도 기록 켜짐. **`t`는 한국시간 `YYYY-MM-DD HH:mm:ss.SSS`**. 한국 자정마다 `serverlog.log` → `serverlog-YYYYMMDD.log` 로 롤링. 민감 정보 주의, 끌 때 **`=0`**. |
| `LOG_RETENTION_DAYS` | `5` | **보관 주기(일)**. `log/` 아래 **`serverlog-*.log`**, 구형 **`logYYYYMMDD.log`**, **`evaluate-warm-payload-*.log`** 중 **mtime** 기준으로 이 값보다 오래된 파일만 삭제합니다. **`0`** 이면 자동 삭제 안 함. 활성 **`serverlog.log`** 는 삭제하지 않습니다. |
...
`WARM_START_URL`을 설정하면 기동 시(및 브라우저 재시작 후) 해당 URL로 이동한 뒤 EverSafe VM을 주입하고 **웜 페이지**를 유지합니다. 이후 **`POST /api/v1/ever-safe/evaluate/warm`** 으로 같은 페이지에서 XHR 단계만 호출할 수 있습니다. 웜 탭이 아직 준비되지 않은 경우 이 API는 **내부에서 `warm/retry`와 동일한 웜 재구성(`setupWarm`)을 한 번 시도한 뒤 XHR을 재실행**합니다(자동 복구). 수동 **`POST /api/v1/ever-safe/warm/retry`** 는 그대로 유효합니다.
...
| `WARM_LOCK_BLOCK_REQUEST_URL_PATTERNS` | 미설정 | 쉼표로 구분한 부분 문자열. 매칭되는 요청은 빈 200(스크립트)/204(문서)로 응답해 로딩 자체 차단. 진단 로그(`[warm-lock] BLOCKED main nav → ... \| initiator: ...`)에서 redirect 트리거 리소스를 식별 후 추가. |
| `WARM_FREEZE_WHEN_IDLE` | 미설정 | `1`이면 워밍 완료 후 페이지를 CDP `Page.setWebLifecycleState({state:'frozen'})` 로 동결. 유휴 상태에서 `setInterval`/`setTimeout`/마이크로태스크 정지 → CPU 사용 0 수렴, 메모리 누적 정지. **`/evaluate/warm`** 호출 시 자동으로 `'active'` 로 깨운 뒤 실행, 종료 후 다시 `'frozen'` 으로 복귀(직렬화된 락 안에서 처리). 백그라운드 토큰 갱신·하트비트가 필요한 사이트라면 검증 후 사용. |
...
| `/api/v1/ever-safe/evaluate/warm` | POST | 웜 페이지에서 **XHR만** (`targetUrl`+`payload`, `url`/`vmLoadBaseUrl` 불가). **`payload`는 JSON 객체가 아니라 인코딩된 문자열**만 허용(아래 인코딩 절 참고). 웜 미준비 시 **서버가 한 번 웜 재구성 후 재시도**하며, 그래도 실패·웜 비활성 등이면 **503**. **응답 본문에 `GET /health`와 동일 필드의 `health` 객체**가 항상 포함되어, 로드밸런서로 `/health`와 다른 인스턴스를 치는 문제 없이 **이 요청을 처리한 인스턴스**의 상태를 바로 확인할 수 있음. |
...
| `warmLockToStartUrl` | boolean | 웜 페이지의 메인 프레임 네비를 `WARM_START_URL` 로만 잠그는지 여부. 본 서버는 항상 `true`. |
| `warmFreezeWhenIdle` | boolean | `WARM_FREEZE_WHEN_IDLE=1` 설정 시 `true`. 워밍 후 유휴 시 페이지가 `frozen` 으로 동결됨을 의미. |
| `warmLifecycleState` | string | 웜 페이지의 현재 라이프사이클 상태. 일반적으로 `"active"` 또는 `"frozen"`. `WARM_FREEZE_WHEN_IDLE` 설정 시 `/evaluate/warm` 호출 사이에 `"frozen"`. |
...
**전제**: `WARM_START_URL` 설정. 정상 경로에서는 **`warmReady: true`** 인 웜 전용 탭에서 **`targetUrl` + `payload`** 로 **XHR 단계만** 실행합니다(`lib/evaluate.js`의 XHR 경로와 동일). VM·`goto`는 웜 구성 단계에서 이미 끝난 상태를 가정합니다.

**자동 복구**: 첫 실행 시 내부적으로 `warm page is not ready`에 해당하는 상태면, **`POST /warm/retry`와 동일한 `setupWarm`(goto + VM 재주입)** 을 **한 번** 수행한 뒤 같은 요청 본문으로 **`evaluateWarm`을 다시 호출**합니다. 재구성이 `skipped` / `busy` / 실패이거나, 재시도 후에도 실패하면 **503**(또는 메시지에 따라 **400**)으로 떨어집니다. `LOG_EVALUATE_WARM_PAYLOAD`가 켜져 있으면 `log/serverlog.log`에 **`phase: "warm_auto_recover"`** NDJSON이 남고, 콘솔에는 `[warm] evaluate/warm: page not ready — running setupWarm (auto-recover)` 한 줄이 출력될 수 있습니다. `warm_auto_recover`의 **`step`** 예: `not_ready` → (`setup_succeeded`·`retrying_evaluate` 또는 `setup_skipped` / `setup_busy` / `setup_failed`) → 성공 시 `evaluate_retry_ok`, 실패 시 `evaluate_retry_failed`.

**`warm_auto_recover` 로그 `step` 요약**

| `step` | 의미 |
|--------|------|
| `not_ready` | 첫 `evaluateWarm`이 미준비로 실패. `stateBefore` 등 |
| `setup_skipped` / `setup_busy` | 재구성 생략·다른 구성 진행 중 → `outcome: "aborted"` |
| `setup_failed` | 재구성 실패 → `setupError`, `stateAfter` |
| `setup_succeeded` | 재구성 성공 후 재평가 진행 → `outcome: "retrying_evaluate"` |
| `evaluate_retry_ok` | 재시도 XHR 성공 → `outcome: "success"` |
| `evaluate_retry_failed` | 재시도도 실패 → `outcome: "failed"` |
...
- **Node(서버·검증용)**: `lib/payloadCustomBase64.js` — `stringToUtf8Bytes`, `utf8BytesToString`, `encodeCustomBase64`, `decodeCustomBase64`, `prepareEvaluateWarmBody`, `ALPHABET_LEN`(`65`). **`lib/healthPayload.js`** 의 `buildHealthSnapshot` — `GET /health` 본문과 `POST /evaluate/warm`의 `health` 필드에 공통 사용.
...
| `health` | **`GET /api/v1/ever-safe/health`와 동일 스키마**. 이 `evaluate/warm` 요청을 **처리한 동일 Node 프로세스**의 스냅샷이므로, Nginx 등으로 `/health`와 `/evaluate/warm`이 다른 백엔드로 갈 때 생기는 **상태 불일치**를 피하려면 클라이언트는 **`health.warmReady` 등을 이 필드만 보고** 판단하면 됨. |
...
| 503 | 웜 비활성(`warm is not enabled`), 자동 복구 후에도 웜 미준비·재구성 실패, 또는 자동 복구 전과 동일한 **미준비**가 남은 경우 등. 메시지에 `auto-recover: warm setup failed` 가 붙으면 **한 번 재구성을 시도했으나 실패**한 것입니다. |

**오류 응답 본문**: **400·503** 등에서도 **`{ "error": "…", "health": { … } }`** 형태로 **`health`는 항상 포함**됩니다(역시 **이 요청을 처리한 인스턴스** 기준). 외부에서 `warm/retry`를 다시 호출할지 여부는 `health`를 보고 결정하면 됩니다.
...
**파일 로그** (`LOG_EVALUATE_WARM_PAYLOAD`가 켜진 경우): `evaluate/warm`과 같이 **`http_request`**(요청 수신), 성공 시 **`http_response`**(`httpStatus: 200`, `body`에 `ok`·`state`), 실패 시 **`http_error`**(403·400·429·500)가 `process.cwd()/log/serverlog.log` NDJSON에 기록됩니다.
...
| **웜 세션** | `lib/warmSession.js` — `WARM_START_URL` 시 전용 페이지에 goto+VM, `POST /api/v1/ever-safe/evaluate/warm`(XHR만·미준비 시 **자동 재구성 1회 후 재시도**), `POST /api/v1/ever-safe/warm/retry` — 라우팅·로그·자동 복구는 `routes/warm.js` |
