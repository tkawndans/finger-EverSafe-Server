...
| `BROWSER_ADMIN_TOKEN` | 미설정 | 설정 시 `POST /api/v1/ever-safe/browser/headful`, **`POST /api/v1/ever-safe/warm/retry`** 에 헤더 `X-Browser-Admin-Token: <값>` 필요. 미설정이면 해당 엔드포인트도 토큰 없이 호출 가능. |
...
| `/api/v1/ever-safe/health` | GET | 서버 상태 확인 (`headful`, **`warmEnabled` / `warmReady` / `warmLastError` / `warmSetupInFlight` / `warmUrl`** 포함) |
| `/api/v1/ever-safe/evaluate/warm` | POST | 웜 페이지에서 **XHR만** (`targetUrl`+`payload`, `url`/`vmLoadBaseUrl` 불가). 웜 미준비 시 503. |
...
- **Admin 토큰**: 입력 시 `POST /api/v1/ever-safe/browser/headful`, `POST /api/v1/ever-safe/warm/retry` 등에 `X-Browser-Admin-Token`으로 전달됨
...
```go
// GET /api/v1/ever-safe/health — warmReady 가 true 일 때만 evaluate/warm 권장
