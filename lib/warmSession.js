...
function getWarmConfig() {
  ...
  /**
   * 본 서버는 워밍 페이지에 대해 **항상 WARM_START_URL 외의 메인 프레임 네비를 차단**한다.
   * (별도 환경변수 토글 없음 — 기본 동작.) 다른 prefix 환경변수는 무시된다.
   */
  const lockToStartUrl = true;
  const blocked = [];
  const allowed = [startUrl];
  ...
  /**
   * `WARM_FREEZE_WHEN_IDLE=1` 이면 워밍 완료 직후 페이지를 CDP `Page.setWebLifecycleState({state:'frozen'})` 로 동결.
   * `/evaluate/warm` 호출 시 잠시 깨워서(`'active'`) 실행 후 다시 동결한다.
   * 효과: 유휴 상태에서 setInterval/setTimeout/마이크로태스크가 모두 정지 → CPU 사용 0 수렴, 누적 메모리 증가도 멈춤.
   * 트레이드오프: 토큰 갱신·하트비트 등 백그라운드 갱신이 있는 사이트라면 갱신이 멎을 수 있어 검증 필요.
   */
  const freezeWhenIdle = process.env.WARM_FREEZE_WHEN_IDLE === "1";
...
let warmCdp = null;
/**
 * 페이지 라이프사이클 상태(권장: 'active' | 'frozen').
 * `WARM_FREEZE_WHEN_IDLE=1` 이 아닐 때는 항상 'active' 로 유지.
 */
let warmLifecycleState = "active";
let warmReady = false;
let warmLastError = null;
let warmSetupInFlight = false;
let lockChain = Promise.resolve();

async function ensureWarmCdp(page) {
  if (warmCdp) return warmCdp;
  if (!page) return null;
  try {
    warmCdp = await page.target().createCDPSession();
    return warmCdp;
  } catch (e) {
    warmCdp = null;
    console.warn("[warm] CDP session attach failed:", e && e.message ? e.message : e);
    return null;
  }
}

async function setLifecycleState(state) {
  if (!warmPage) return false;
  const cdp = await ensureWarmCdp(warmPage);
  if (!cdp) return false;
  try {
    await cdp.send("Page.setWebLifecycleState", { state });
    warmLifecycleState = state;
    return true;
  } catch (e) {
    console.warn(`[warm] setWebLifecycleState(${state}) failed:`, e && e.message ? e.message : e);
    return false;
  }
}

async function freezeWarmPage() {
  if (warmLifecycleState === "frozen") return true;
  const ok = await setLifecycleState("frozen");
  if (ok) console.log("[warm] page frozen (idle)");
  return ok;
}

async function unfreezeWarmPage() {
  if (warmLifecycleState === "active") return true;
  const ok = await setLifecycleState("active");
  if (ok) console.log("[warm] page active (running)");
  return ok;
}
...
function getState() {
  ...
  try {
    ...
    return {
      ...
      warmSetupInFlight,
      warmUrl: null,
      warmLockToStartUrl: false,
      warmFreezeWhenIdle: false,
      warmLifecycleState: warmLifecycleState,
  ...
  return {
    ...
    warmLockToStartUrl: !!(cfg.enabled && cfg.lockToStartUrl),
    warmFreezeWhenIdle: !!(cfg.enabled && cfg.freezeWhenIdle),
    warmLifecycleState,
  };
}
...
async function setupWarm(deps) {
  ...
  try {
    ...
    const page = await createPage(
      cfg.lockToStartUrl ? { skipNavigationBlockRecovery: true } : undefined
    );
    warmPage = page;
    ...
    await nav.applyNavigationPolicy(
      ...
      cfg.blocked,
      cfg.allowed,
      cfg.lockToStartUrl ? cfg.startUrl : undefined
    );
    ...
    /** WARM_FREEZE_WHEN_IDLE=1: 워밍 직후 동결 → 유휴 상태에서 CPU 0 수렴 */
    if (cfg.freezeWhenIdle) {
      await freezeWarmPage().catch(() => {});
    }
 ...
 *
 * `WARM_FREEZE_WHEN_IDLE=1` 인 경우 호출 직전 active 로 깨우고, 종료 후 frozen 으로 복귀.
 */
async function evaluateWarm(body, timeout, deps) {
  return withLock(async () => {
    ...
    if (cfg.freezeWhenIdle) await unfreezeWarmPage().catch(() => {});
    try {
      return await executeOnPage(warmPage, b, timeout || 30_000, { gotoAndWait });
    } finally {
      if (cfg.freezeWhenIdle) await freezeWarmPage().catch(() => {});
    }
  });
}
