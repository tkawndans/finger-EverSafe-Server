...
async function executeOnPage(page, body, timeout, deps = {}) {
  ...
  if (targetUrl && payload !== undefined) {
    if (warmPayloadLog.isEnabled()) {
      const ct = contentType || "application/json";
      const sendBody = serializeBody(payload, ct);
      const max = 4000;
      const preview =
        sendBody.length > max
          ? `${sendBody.slice(0, max)}…(truncated, total ${sendBody.length} chars)`
          : sendBody;
      const row = {
        targetUrl,
        contentType: ct,
        utf8ByteLength: Buffer.byteLength(sendBody, "utf8"),
        preview,
      };
      console.log("[executeOnPage xhr.send string]", row);
      try {
        warmPayloadLog.append({ phase: "xhr_request", ...row });
      } catch (_) {}
    }
    const delegate =
    ...
    if (warmPayloadLog.isEnabled() && xhrResult != null) {
      try {
        warmPayloadLog.append({
          phase: "xhr_response",
          targetUrl,
          summary: warmPayloadLog.truncateJsonString(xhrResult, 16_000),
        });
      } catch (_) {}
    }
  }
