 ...
 *
 * 보관: LOG_RETENTION_DAYS (기본 5). 지난 일수보다 오래된 serverlog-*.log, 구형 logYYYYMMDD.log 등만 삭제.
...
/** 보관 주기(일). 미설정·빈 값 → 5. 0 → 자동 삭제 안 함. */
function getRetentionDays() {
  const v = process.env.LOG_RETENTION_DAYS;
  if (v === undefined || v === null || String(v).trim() === "") return 5;
